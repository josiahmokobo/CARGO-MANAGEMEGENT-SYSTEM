...
<div id="contents">
     ...
     if(($_POST["password1"]=="") || preg_match('/^([a-zA-Z0-9]){5,16}$/',$_POST["password1"])=='0')
		  ...
		  if((strcmp($Newpassword, $Confirmpassword))!=0)
		   ...
		   {
		   $sql=mysql_query("select password,Username phonenumber from user where  Username='$username'  and PhoneNumber='$phone'") or die(mysql_error());
		  $num=mysql_num_rows($sql);
		  $row=mysql_fetch_array($sql);
		   if($num<'1')
		   {
		   echo "username is wrong".mysql_error();
		   }
		   else
		   {
